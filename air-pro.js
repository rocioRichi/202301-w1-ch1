...
function averageCost() {
  ...
  alert(
    `El coste medio de los vuelos es de ${(average / flights.length).toFixed(
      2
    )} €`
  );
}
...
const youAreAdmin = () => {
  let role = prompt(
    " Quieres registrar(R) o eliminar (E) un nuevo vuelo?. Para salir (#)"
  );
  role = removeAccentsLowerCase(role);
...
function deleteFlight() {
  let idIntroduced = +prompt(
    `Introduzca el número de ID (0X) del vuelo a eliminar. Si el Id es correcto, confirmaremos la eliminación del registro`
  );
  ...
  for (const key in flights) {
    if (flights.hasOwnProperty.call(flights, key)) {
      ...
      if (element.id === idIntroduced) {
        if (
          window.confirm(
            `Vas a eliminar el vuelo con id = ${flights[idIntroduced].id} con origen ${flights[idIntroduced].from} y destino ${flights[idIntroduced].to}`
          )
        ) {
